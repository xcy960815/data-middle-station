...
export class TableStickyPlugin {
  ...
  private checkHeaderElementFixed(option: TableSticky.Option): boolean {
    const tableHeaderElement = this.getElementByClassName('.el-table__header-wrapper', true, option)
   ...
   * @returns { HTMLElement }
   */
  private getElementByClassName(className: string, inTable?: boolean, option?: TableSticky.Option): HTMLElement {
    if (inTable) {
  ...
  private handleScrollElementOnScroll(option: TableSticky.Option): void {
    const { scrollElement, fixedTopValue, tableHeaderElementOriginalTop } = this.getStickyConfig(option)
    // 滚动条距离body顶部的距离
   ...
   */
  // @Debounce(300, true)
  private updateStickyConfig(option: TableSticky.Option, stickyConfig: Partial<TableSticky.StickyConfig>): void {
    const uid = this.getUid(option)
   ...
   */
  private getWillChangeElementsResizeObserver(option: TableSticky.Option): Array<ResizeObserver> {
    const { willBeChangeElementClasses } = option.binding.value
    if (!willBeChangeElementClasses) return []
    return willBeChangeElementClasses.map((willBeChangeElementclass: string, index: number) => {
      ...
      const willChangeElementResizeObserver = new ResizeObserver((entries) => {
        ...
        const { willChangeElementsOriginalHeight, tableHeaderElementOriginalTop } = this.getStickyConfig(option)
        ...
        if (originalHeight !== currentHeight) {
          const heightDifference = Number(currentHeight.replace('px', '')) - Number(originalHeight.replace('px', ''))
          // 获取当前表头是否被fixed
          const isFixed = this.checkHeaderElementFixed(option)
  ...
  private initStickyConfig(option: TableSticky.Option): TableSticky.StickyConfig {
    const tableInnerWapperElement = this.getElementByClassName('.el-table__inner-wrapper', true, option)
    const tableHeaderElement = this.getElementByClassName('.el-table__header-wrapper', true, option)
    ...
    const { binding } = option
    const scrollElementClassName = binding.value && binding.value.parent ? binding.value.parent : 'body'
    // 获取滚动的节点
  ...
  unmounted(option: TableSticky.Option): void {
    const { handleScrollElementOnScroll, scrollElement, tableElementResizeObserver, willChangeElementsResizeObserver } =
      this.getStickyConfig(option)
...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  nuxtApp.vueApp.directive('sticky', {
    mounted(tableElement: HTMLElement, binding: TableSticky.TableStickyBinding, vnode: VNode) {
      tableStickyPlugin.mounted({ tableElement, binding, vnode })
    },
    updated(tableElement: HTMLElement, binding: TableSticky.TableStickyBinding, vnode: VNode) {
      tableStickyPlugin.updated({ tableElement, binding, vnode })
    },
    unmounted(tableElement: HTMLElement, binding: TableSticky.TableStickyBinding, vnode: VNode) {
