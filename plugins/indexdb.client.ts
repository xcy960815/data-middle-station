...
class IndexDBManager {
   ...
   */
  addData<T extends IndexDB.DataRecord>(database: IDBDatabase, storeName: string, dataList: T[]) {
    const store = database.transaction([storeName], 'readwrite').objectStore(storeName)
   ...
   */
  addDataAsync<T extends IndexDB.DataRecord>(database: IDBDatabase, storeName: string, dataList: T[]): Promise<void> {
   ...
   */
  newAddData<T extends IndexDB.DataRecord>(database: IDBDatabase, storeName: string, dataList: T[]): Promise<void> {
    return this.addDataAsync<T>(database, storeName, dataList)
  }
   ...
   */
  updateData<T extends IndexDB.DataRecord>(database: IDBDatabase, storeName: string, dataList: T[]): Promise<boolean> {
    return new Promise((resolve) => {
      const transaction = database.transaction([storeName], 'readwrite')
      const store = transaction.objectStore(storeName)
      let completedCount = 0
   ...
   */
  getData<T extends IndexDB.DataRecord>(
    database: IDBDatabase,
    storeName: string,
    ...
    return new Promise((resolve) => {
      request.onsuccess = function (e: Event) {
        resolve((e.target as IDBRequest<T | undefined>).result)
      }
    })
  }
   ...
   */
  getDataByPage<T extends IndexDB.DataRecord>(
    database: IDBDatabase,
    storeName: string,
    indexName: string,
    ...
    page?: number,
    pageSize?: number
  ): Promise<IndexDB.QueryResult<T>> {
    const data: IndexDB.QueryResult<T> = {
      total: 0,
      rows: []
    }
    ...
    return new Promise((resolve) => {
      request.onsuccess = function (e: Event) {
        ...
        if (page !== undefined && pageSize !== undefined) {
          ...
          data.rows.push(cursor.value as T)
   ...
   */
  getDataByone<T extends IndexDB.DataRecord>(
    database: IDBDatabase,
    storeName: string,
    indexName: string,
    searchValue: IDBValidKey
   ...
   */
  getAllData<T extends IndexDB.DataRecord>(database: IDBDatabase, storeName: string): Promise<T[]> {
    const objectStore = database.transaction(storeName, 'readonly').objectStore(storeName)
    const request = objectStore.getAll()

    return new Promise((resolve) => {
      request.onsuccess = function (e: Event) {
        resolve((e.target as IDBRequest<T[]>).result as unknown as T[])
      }
    })
  }
   ...
   */
  getDataByoneWithSort<T extends IndexDB.DataRecord>(
    database: IDBDatabase,
    storeName: string,
    indexName: string,
    searchValue: IDBValidKey,
    ...
    return new Promise((resolve, reject) => {
      request.onsuccess = function (e: Event) {
        ...
        if (cursor) {
          ...
          data.sort((a, b) => {
            const aValue = (a as IndexDB.DataRecord)[sortField] as string
            const bValue = (b as IndexDB.DataRecord)[sortField] as string
   ...
   */
  checkAndAdd<T extends IndexDB.DataRecord>(
    database: IDBDatabase,
    storeName: string,
    fieldName: string,
    ...
    return new Promise((resolve, reject) => {
      this.getDataByone<T>(database, storeName, fieldName, fieldValue)
        .then((res) => {
          if (res && res.length > 0) {
            resolve('已存在')
          } else {
            this.addData<T>(database, storeName, [dataRecord])
            resolve('添加成功')
          }
        })
        .catch((err) => {
   ...
   */
  getEarliestByCreateTime<T extends IndexDB.DataRecord>(database: IDBDatabase, storeName: string): Promise<T | null> {
    return new Promise((resolve, reject) => {
      this.getAllData<T>(database, storeName)
        .then((allData) => {
          if (allData.length === 0) {
            resolve(null)
            return
          ...
          const sorted = allData.sort((a, b) => {
            const aValue = (a as IndexDB.DataRecord).createTime as string
            const bValue = (b as IndexDB.DataRecord).createTime as string
   ...
   */
  getDataByConditions<T extends IndexDB.DataRecord>(
    database: IDBDatabase,
    storeName: string,
