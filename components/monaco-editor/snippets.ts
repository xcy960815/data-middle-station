...
import type { FieldOption, DatabaseOption, TableOption, SortText, SuggestOption } from 'monaco-editor'
    ...
    if (textBeforeLastToken.endsWith('.')) {
      ...
      const databaseOption = this.databaseOptions.find(
        (databaseOption: DatabaseOption) =>
          databaseOption.databaseName.toLowerCase() === textBeforeLastTokenNoDot.replace(/^.*,/g, '')
      )
      if (databaseOption) {
        // <库名>.<表名> 联想
      ...
      ['select', 'where', 'order by', 'group by', 'by', 'and', 'or', 'having', 'distinct', 'on'].includes(
        textBeforeLastToken.replace(/.*?\(/g, '')
      ) ||
      (textBeforeLastToken.endsWith('.') &&
        !this.databaseOptions.find(
          (databaseOption: DatabaseOption) => `${databaseOption.databaseName}.` === textBeforeLastToken
        )) ||
