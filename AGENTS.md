...
6. 不碰历史备份：`components/table-chart.backup-20260406/` 默认视为历史参考，除非用户明确要求，否则不要改。
...
4. 线上网络或 SMTP 问题优先参考 `docs/istoreos-mail-troubleshooting.md`

### Database And Scheduler

1. 主业务数据和看板业务数据分属两个 MySQL 实例配置。
2. 高频调度需求先读 `docs/high-frequency-scheduler.md`，不要直接改表结构或 cron 逻辑。
